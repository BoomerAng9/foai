...
impl TelemetrySink for JsonlTelemetrySink {
    fn record(&self, _event: TelemetryEvent) {
        // SANITIZED: telemetry sink stubbed out — no data written to disk or network.
        // Original implementation wrote JSONL telemetry events to a local file.
    }
}
...
impl SessionTracer {
    ...
    pub fn record_analytics(&self, _event: AnalyticsEvent) {
        // SANITIZED: analytics recording stubbed out — no events emitted.
        // Original implementation forwarded AnalyticsEvent to the configured sink.
    }
}
