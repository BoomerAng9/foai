...
import { generateMockDraft, type ProspectInput } from '@/lib/draft/mock-engine';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const picks = generateMockDraft(prospects, { rounds });

    return NextResponse.json({ picks, rounds, totalPicks: picks.length });
  } catch (error) {
