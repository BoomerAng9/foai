...
$base = "c:\Users\rishj\OneDrive\Desktop\A.I.M.S\frontend"
$extensions = "*.tsx", "*.ts"

$files = Get-ChildItem -Path $base -Recurse -Include $extensions | Where-Object { $_.FullName -notmatch "node_modules" }

# Ordered replacements (same as before, plus any new ones)
...
foreach ($file in $files) {
    ...
    if ($fileChanges -gt 0) {
        ...
        $filesChanged++
        Write-Host "UPDATED ($fileChanges changes): $($file.FullName)"
    }
}
