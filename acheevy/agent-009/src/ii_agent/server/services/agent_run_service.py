...
class AgentRunService:
    ...
    def get_routing_decision(
        task: AgentRunTask | AgentRunTaskResponse | None,
    ) -> dict[str, Any]:
        if task is None or not isinstance(task.routing_decision, dict):
            return {}
        return dict(task.routing_decision)

    @classmethod
    def get_execution_lane(
        cls, task: AgentRunTask | AgentRunTaskResponse | None
    ) -> str:
        routing_decision = cls.get_routing_decision(task)
        execution_lane = routing_decision.get("execution_lane")
        return execution_lane if isinstance(execution_lane, str) else "direct_ii_agent"
    ...
    def build_runtime_metadata(
        ...
        *,
        ...
        return ExecutionLaneService.build_runtime_metadata(
            routing_decision=cls.get_routing_decision(task),
    ...
    async def get_task_by_id(
        ...
        res = AgentRunTaskResponse(
            ...
            routing_decision=task_run.routing_decision,
    ...
    async def update_task_status(
        ...
        return AgentRunTaskResponse(
            ...
            routing_decision=task_to_update.routing_decision,
