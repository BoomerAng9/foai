...
from ii_agent.server.models.session_context import merge_session_settings_payload
...
class ChatService:
    ...
    @classmethod
    async def persist_request_session_settings(
        cls,
        *,
        ...
        """Merge request-scoped session metadata into the durable session record."""
        result = await db_session.execute(
            select(Session).where(Session.id == session_id, Session.user_id == user_id)
        )
        session = result.scalar_one()

        derived_routing_decision = None
        if chat_request.routing_decision is None:
            derived_routing_decision = derive_routing_decision_payload(
                user_text=extract_user_text_from_chat_content(chat_request.content),
                build_spec=(
                    chat_request.build_spec.model_dump(exclude_none=True)
                    if chat_request.build_spec is not None
                    else None
                ),
                attachment_ids=chat_request.file_ids,
        ...
        merged_settings = merge_session_settings_payload(
            session.settings,
            session_snapshot=(
                chat_request.session_snapshot.model_dump(exclude_none=True)
                if chat_request.session_snapshot is not None
                else None
            ),
            session_context=(
                chat_request.session_context.model_dump(exclude_none=True)
                if chat_request.session_context is not None
                else None
            ),
            build_spec=(
                chat_request.build_spec.model_dump(exclude_none=True)
            ...
            routing_decision=(
                chat_request.routing_decision.model_dump(exclude_none=True)
                if chat_request.routing_decision is not None
                else derived_routing_decision
            ),
        )

        if merged_settings is not None and merged_settings != (session.settings or {}):
    ...
    async def stream_chat_response(
        ...
        await cls.persist_request_session_settings(
            db_session=db_session,
            session_id=session_id,
            user_id=user_id,
        ...
        routing_decision_payload = (
            chat_request.routing_decision.model_dump(exclude_none=True)
            if chat_request.routing_decision is not None
            else derive_routing_decision_payload(
                user_text=extract_user_text_from_chat_content(chat_request.content),
                build_spec=(
                    chat_request.build_spec.model_dump(exclude_none=True)
                    if chat_request.build_spec is not None
