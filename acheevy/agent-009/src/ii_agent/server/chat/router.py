...
# NTNTN+SME_ANG imports
from ii_agent.utils.ntntn_converter import (
    ntntn_translate,
    needs_clarification as ntntn_needs_clarification,
)
from ii_agent.utils.sme_ang_generator import (
    clarification_question_for,
    get_clarification_engine_label,
)
from ii_agent.server.models.session_context import RoutingDecisionInfo
from ii_agent.server.services.execution_lane_service import ExecutionLaneService
from ii_agent.server.services.routing_service import (
    derive_routing_decision_payload,
    extract_user_text_from_chat_content,
)
...
def _build_sse_response(generator):
...
async def send_chat_message(
    ...
    **Event: tool_result** (tool execution result)
    ...
    try:
        await ChatService.persist_request_session_settings(
            db_session=db_session,
            session_id=session_id,
            user_id=str(current_user.id),
        ...
        logger.error(f"Failed to persist session envelope for {session_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to persist session metadata")

    user_content_text = extract_user_text_from_chat_content(request.content)

    if request.routing_decision is None and user_content_text:
        derived_routing_decision = derive_routing_decision_payload(
            user_text=user_content_text,
            build_spec=(
                request.build_spec.model_dump(exclude_none=True)
                if request.build_spec is not None
                else None
            ),
            attachment_ids=request.file_ids,
            tools=request.tools,
        )
        if derived_routing_decision is not None:
            request.routing_decision = RoutingDecisionInfo.model_validate(
                derived_routing_decision
            )

    lane_plan = ExecutionLaneService.build_plan(
        request.routing_decision.model_dump(exclude_none=True)
    ...
    if not lane_plan.runtime_execution_allowed:
        async def blocked_generator():
            error_event = {
                "status": "error",
                "error": lane_plan.policy_reason
                or "Requested execution lane is not supported by the runtime.",
                "code": "unsupported_execution_lane",
                "execution_policy": lane_plan.to_metadata(),
            }
            yield f"event: error\ndata: {json.dumps(error_event)}\n\n"
            yield f"event: done\ndata: {{}}\n\n"

        return _build_sse_response(blocked_generator())
    
    # Run NTNTN translation and check for clarification need
    if user_content_text.strip():
        technical_translation = ntntn_translate(user_content_text)
        
        if ntntn_needs_clarification(user_content_text, technical_translation):
            # Generate clarification question
            clarification_question = clarification_question_for(user_content_text)
            
            # Return early with clarification event
            ...
            return _build_sse_response(clarification_generator())
    ...
    async def event_generator():
        ...
        try:
            ...
            if lane_plan.policy_action == "fallback_to_direct":
                policy_event = {
                    "event": "execution_policy",
                    "status": "info",
                    "execution_policy": lane_plan.to_metadata(),
                }
                yield f"event: message\ndata: {json.dumps(policy_event)}\n\n"
