...
class EncryptionManager:
    ...
    def _get_or_generate_key(self) -> bytes:
        ...
        password_str = os.getenv("ENCRYPTION_PASSWORD", "")
        ...
        if not password_str or not salt_str:
            ...
            if environment == "production":
                raise RuntimeError(
                    "ENCRYPTION_PASSWORD and ENCRYPTION_SALT must be set "
                    "in production. Generate with: python -c "
                    "'import secrets; print(secrets.token_urlsafe(32))'"
                )
            # Dev/test fallback only — never used in production
            password_str = password_str or "dev-only-insecure-password"
            salt_str = salt_str or "dev-only-insecure-salt"
