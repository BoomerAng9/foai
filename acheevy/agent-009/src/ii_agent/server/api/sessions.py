...
from ii_agent.server.models.session_context import validate_session_settings_payload
...
async def update_session(
    ...
    """Update mutable session fields for the authenticated user."""
    try:
        update_payload = session_update.model_dump(exclude_unset=True)

        if "settings" in update_payload:
            update_payload["settings"] = validate_session_settings_payload(
                update_payload.get("settings")
            )

        session_data = await Sessions.update_session(
            session_id,
            str(current_user.id),
            **update_payload,
        )
