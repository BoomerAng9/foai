...
from ii_agent.server.services.routing_service import (
    ...
    resolve_agent_type_for_routing,
)
...
class UserQueryHandler(CommandHandler):
    ...
    async def _send_execution_lane_blocked_event(
        self,
        *,
        ...
        await self.send_event(
            RealtimeEvent(
                ...
                content={
                    "message": message,
                    "error_type": "unsupported_execution_lane",
                },
            )
        )
    ...
    async def _persist_session_routing_decision(
        self,
        *,
        session_id: uuid.UUID,
        user_text: str,
        ...
        async with get_db_session_local() as db:
            session = await Sessions.find_session_by_id(db=db, session_id=session_id)
            if not session:
                return

            await persist_session_routing_decision(
                db_session=db,
                session=session,
                user_text=user_text,
    ...
    async def handle(
        ...
        # NTNTN+SME_ANG Clarification Check (optional integration point)
        user_text = query_command.text.strip()
        if user_text:
            await self._persist_session_routing_decision(
                session_id=existing_session.id,
                user_text=user_text,
                attachment_ids=query_command.files,
            )

        if user_text:
            technical_translation = ntntn_translate(user_text)
            
            if ntntn_needs_clarification(user_text, technical_translation):
                # Generate clarification question
                clarification_question = clarification_question_for(user_text)
                
                # Emit clarification required event and return early
                await self.send_event(
                    RealtimeEvent(
            ...
            async with lock_instance as _:
                async with get_db_session_local() as db:
                    ...
                    running_task = await AgentRunTask.create(
                        ...
                        routing_decision=(
                            current_session.settings.get("routing_decision")
                            if current_session
                            and isinstance(current_session.settings, dict)
                            else None
                        ),
                    )
                    ...
                    lane_plan = ExecutionLaneService.build_plan(
                        AgentRunService.get_routing_decision(running_task)
                    )
                    ...
                    if not lane_plan.runtime_execution_allowed:
                        ...
                        await db.commit()
                        await self._send_execution_lane_blocked_event(
                            session_id=session_info.id,
                            run_id=running_task.id,
                            message=lane_plan.policy_reason
    ...
    async def _init_chat_session(
        ...
        base_metadata = AgentRunService.build_runtime_metadata(
            task=agent_task,
        ...
        resolved_agent_type = resolve_agent_type_for_routing(
            requested_agent_type=session.agent_type,
            routing_decision=AgentRunService.get_routing_decision(agent_task),
            user_text=user_query,
        )
        merged_metadata["resolved_agent_type"] = resolved_agent_type.value
        ...
        if config.policy_layers_emit_debug_events:
            lane_plan = ExecutionLaneService.build_plan(
                AgentRunService.get_routing_decision(agent_task)
            )
