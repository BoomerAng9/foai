...
export async function GET(req: NextRequest) {
  if (!TOKEN) {
    return NextResponse.json(
      { error: "Pooler Pass cadence pricing not configured. Owner: set COASTAL_GATEWAY_TOKEN on coastal-web." },
      { status: 503 },
    );
  }
  ...
  try {
    runnerRes = await fetch(
      `${RUNNER_BASE}/api/membership/pooler-pass/cadence-pricing?tier=${encodeURIComponent(tier)}`,
      {
        method: "GET",
