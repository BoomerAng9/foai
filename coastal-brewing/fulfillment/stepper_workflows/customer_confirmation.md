...
3. Build a task packet:
   ```json
   {
     ...
     "desired_output": "email transmitted; Hermes action receipt"
   }
   ```
...
6. Once live (post-Phase-5), Stepper sends via the configured channel (SendGrid / Resend / Hostinger Mail) and writes a Hermes `action_receipts` row with `status='sent'` + transmission details.

## Outputs
- Hermes route receipt
- (post-live) Hermes `action_receipts` row with status `sent`
...
## Failure modes
- Email channel rejects the send (bounce, hard fail) → Stepper writes failure to Hermes, notifies Boomer_CX, no retry without owner approval
